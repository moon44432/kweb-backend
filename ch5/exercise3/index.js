...
app.get('/fare', async(req, res, next) => {
    try {
        const {uid} = req.query;
        const sql =
            'SELECT users.name, ' + 'Sum(Round(types.fare_rate * trains.distance / 1000, -2)) AS totalFare ' + 'FROM tickets ' +
...
app.get('/train/status', async(req, res, next) => {
    try {
        ...
        const sql =
            ...
            'INNER JOIN types ON trains.type = types.id';
        const {occ, max} = (await runQuery(sql, [tid]))[0];
        const isSeatsLeft = max > occ;
